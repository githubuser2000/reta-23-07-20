...
-   Line specifications are e.g. a number or a range of numbers like 2-5 or these things separated with comma like 1,3-5,20
    For the command "u" and "a", i.e. "universe" and "intent", you can also specify fractions, like 2/3,4/5,1/2.
...
-   Command "u" and "a", i.e. "universe" and "intention", also outputs the reciprocal of the given fractions, e.g. for 2/3,3/4 also 3/2,4/3.
-   Some of the short commands consisting of letters like "a" or "u" can also be used as commands without spaces in between. Example: instead of "a u 1,2" also "au1,2" works.
...
-   "10+2" As an indication, if the multiples were meant (There are then always multiples here at "v10+2" [and not only at inputs for explicit multiples, but then everywhere]), that would be not only 8 and 12, but also 18 and 22, and 28 and 32, and so on.
-   In the readme of reta, instead of retaPrompt, this is explained again
-   5,v20-22 means line 5 and also all multiples of 20,21,22, e.g. 40,42,44
-   20,v10 means all multiples of 10 without the 20 in it
-   with fractions this v syntax works exactly as well
...
-   This syntax for fractions and integers works with the "v" or "multiple" command and with this minus syntax, which is there for taking out ranges.
-   Example
    - The 7 is the good and the 6 the value and both result in the good: 7-6+1 would then have to be the antagonists of it (This is not only 5 and 8, but also 6 and 7 again stupidly). Then 7-6 must be subtracted and the multiples are also searched for, so "v7-6+1,v-6-7" are all antagonists of "Wohl".

### The commands 15\COPY00 and EIG..... {#die-befehle-15-dot-dot-dot-dot}

- The commands beginning with 15\_ together form a tree structure and are output commands of the basic structures and mind (15), as "u" or "a" are output commands. A line specification is needed. Then something can be output.
