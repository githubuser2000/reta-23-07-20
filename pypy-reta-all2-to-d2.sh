...
if [ "$1" == 'tar' ] || [ "$2" == 'tar' ] || [ "$3" == 'tar' ] || [ "$4" == 'tar' ]; then
	echo sende auch tar
	cd ${ABS_DIRECTORY}
	cp -ax {*.{csv,txt,py,js,org,md,png,sh,ts,toml},head*.alx,foot*.alx,retaPrompt,reta,rp*,math,modulo,prim,prim24,generate_html,i18n,*.english,doc,csv,run,libs} /home/alex/myRepos/religions-tabelle-releasses/31
	...
	tar -c {*.{csv,txt,py,js,org,md,png,sh,ts,toml},head*.alx,foot*.alx,retaPrompt,reta,rp*,math,modulo,prim,prim24,generate_html,i18n,*.english,doc,csv,run,libs}  > /home/alex/myRepos/religions-tabelle-releasses/reta.tar
	tar -c {*.{csv,txt,py,js,org,md,png,sh,ts,toml},head*.alx,foot*.alx,retaPrompt,reta,rp*,math,modulo,prim,prim24,generate_html,i18n,*.english,doc,csv,run,libs} | xz - --threads=16 -9 --lzma2=dict=256MiB -c - > ~/reta.tar.xz
